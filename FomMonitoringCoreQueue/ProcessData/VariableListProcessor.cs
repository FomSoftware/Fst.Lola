...
namespace FomMonitoringCoreQueue.ProcessData
{
    public class VariableListProcessor : IProcessor<VariablesList>
    {
        ...
        public VariableListProcessor(IFomMonitoringEntities context, IMessageService messageService)
        {
        ...
        public bool ProcessData(VariablesList data)
        {
            ...
            {
                _context.Refresh();
                var mac = _context.Set<Machine>().FirstOrDefault(m => m.Serial == data.InfoMachine.FirstOrDefault().MachineSerial);
                if (mac == null)
                    return false;
